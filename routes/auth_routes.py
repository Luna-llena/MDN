...
@auth_bp.route('/flask/user', methods=['DELETE'])
def delete_user():
...
@auth_bp.route('/flask/user/profile', methods=['PUT'])
def update_profile():
    data = request.get_json()
    uid = data.get("uid")
    name = data.get("name")
...
@auth_bp.route('/flask/user/body', methods=['PUT'])
...
@auth_bp.route('/flask/logout', methods=['POST'])
def logout():
